...
abstract class ContainerFactory<C : GenericContainer<*>?> {
    ...
    private class ContainerOrException(
        ...
        @Volatile private var _exception: RuntimeException? = null
    ...
    fun shared(
        imageName: String?,
        vararg namedContainerModifiers: NamedContainerModifier<C?>?
    ): C? {
    ...
    private fun createAndStartContainer(
        ...
        LOGGER!!.info(
            ...
            imageName,
            Lists.transform(namedContainerModifiers) { c: NamedContainerModifier<C?>? ->
                c!!.name()
            }
        )
